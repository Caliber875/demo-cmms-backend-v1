...
export class AssetController {
  ...
  static async getAssetStats(req: Request, res: Response): Promise<void> {
    try {
      ...
      const manufacturerStats = await Asset.aggregate([
        {
          $match: { 
            deleted: { $ne: 'Yes' },
  ...
  // Bulk import assets
  static async bulkImportAssets(req: Request, res: Response): Promise<void> {
    try {
